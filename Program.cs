...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            return shows.Where(s => s.StartYear >= 1990 && s.EndYear <= 1999).Select(s => s).ToList();
        }
        ...
        {
            return shows.OrderByDescending(s => s.EndYear).Where(s => s.EndYear == 2000).FirstOrDefault();
        }
        ...
        {
            return string.Join(", ", shows.Count());
        }
